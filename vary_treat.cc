...
int main() {
    ...
    long double phi_1; // initialized later
    unsigned long long n = 2'097'152ULL; // 536'870'912ULL; // 2'097'152ULL; // 1'048'576ULL; // 2^20
    unsigned long long m = 2'097'152ULL; // 67'108'864ULL; // 16'777'216ULL; // 90'699'264ULL; // 3^11 * 2^9
    long double base_a_l = 0.9;
    long double base_a_c = 0.9;
    vector<long double> ks = {};

    ofstream outFile;
    string timeStr = getTime();
    outFile.open("treatment_vary-" + timeStr + ".txt");

    // Print nSteps
    outFile << nSteps << '\n';
    
    long double coeffs[] = { 1.1, 1.2, 1.3, 1.4, 1.5 };

    for (int s = 0; s < nSteps; s++) {
        phi_1 = phi_0 * coeffs[s];
        ...
        for (int thrd = 0; thrd < nThrds; thrd++) {
            thrds.push_back(thread([&](int ti) {
                ...
                Marketplace mp(
                        ...
                        vector<vector<long double> >{vector<long double>{phi_0 * 0.4}, vector<long double>{phi_0 * 1.4}},
                        vector<vector<long double> >{vector<long double>{phi_1 * 0.4}, vector<long double>{phi_1 * 1.4}}
                    );
                /*
                Marketplace mp(
                        n, m,
